    ...
    with
        ...
        static member parse user=
            ...
            if m.Success then
                ...
                failwithf "Could not parse %s" user
            
type BidId = Guid
type AuctionId = Guid
type Amount={ value:float; currency:Currency }
type Auction={ id: AuctionId; title:string; endsAt:DateTime; user:UserId }
    with
        static member getId (auction:Auction)=
            auction.id
type Bid={ id:BidId; auction:Auction; at:DateTime; user:UserId; amount:Amount; retracted:DateTime option }
    with
