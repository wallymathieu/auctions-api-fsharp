...
module ``JwtPayload deserialization spec``=
  ...
  let ``base64 encoded seller`` = decodeXJwtPayloadHeader (Choice1Of2 "eyJzdWIiOiJhMSIsICJuYW1lIjoiVGVzdCIsICJ1X3R5cCI6IjAifQo=")
  [<Fact>]
  let ``can parse base64 and interpret seller``() =
      assertOkWithValue ({ user = BuyerOrSeller (UserId "a1", "Test") }, ``base64 encoded seller``)
  let ``base64 encoded buyer`` = decodeXJwtPayloadHeader (Choice1Of2 "eyJzdWIiOiJhMiIsICJuYW1lIjoiQnV5ZXIiLCAidV90eXAiOiIwIn0K")
  [<Fact>]
